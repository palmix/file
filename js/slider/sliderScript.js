...
function parallaxSlider(options){
	...
	function positioncontrols($controls){
		...
		if (setting.orientation == 'h'){
			controltop = (setting.dimensions[1] > winheight)? winheight/2 : '90px'
			$controls.css({top: controltop, marginTop: -controlheight/2})
		}
		else if (setting.orientation == 'v'){
			controlleft = (setting.dimensions[0] > winwidth)? winwidth/2 : '90px'
...
parallaxSlider.prototype={
	...
	init:function($){
		...
		var $controls =  $('<img class="navbutton" src="'+controlpaths[1]+'" data-dir="forth" style="position: absolute; z-index: 5; cursor: pointer; bottom: 8px; opacity: 0; left: 90px;" />'
			+ '<img class="navbutton" src="'+controlpaths[0]+'" data-dir="back" style="position: absolute; z-index: 5; cursor: pointer; top: 8px; opacity: 0; left: 90px;"/>'
		)
		.css({opacity:0})
		.click(function(){
